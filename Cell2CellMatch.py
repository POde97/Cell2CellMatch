...
class Cell2CellMatch():
  ...
  def __init__(self,adata,batch_key="batch",resolution =1.0,n_clusters =None,ref = None,gpu_cuda =None,strategy_T ="conservative",custom_T=None,strategy_G ="local-topology",mply_p = 16):
    ...
    self.adlist = [CellID(ad=x,verbose =False).ad for x in tqdm.tqdm(adlist)]
    #Labelling
    if ref is not None:
    ...
    if strategy_G == "local-topology":
      ...
      H = Hypergeom(self.adata,self.adata,prediction = False,verbose = True,gpu_cuda = gpu_cuda,mply_p=mply_p)
    ...
    #
      if strategy_T == "percolation":
        T = find_threshold_bfs(np.array(nx.adjacency_matrix(G).todense()))
  ...
  def RunHGT(self):#PvalueMatrixs
    ...
    #CombinationAdata2.append(x2)
          self.PvalMS.append(Hypergeom(x1,x2,prediction = False,verbose = False).Intersection)
...
#from plot import*
    ...
    plot_clustered_stacked(CC,[1.01,0.1],[1.41,0.1],list(dftot[self.batch_key].unique()),title="Local-Topology",colours=color)
